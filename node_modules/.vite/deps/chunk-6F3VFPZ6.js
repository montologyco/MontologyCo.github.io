...
var PinpointEventBuffer = class {
  ...
  push(event) {
    if (this._buffer.length >= this._config.bufferSize) {
      logger10.debug("Exceeded Pinpoint event buffer limits, event dropped.", {
  ...
  _handlePutEventsFailure(err, eventMap) {
    logger10.debug("putEvents call to Pinpoint failed.", err);
  ...
  _processPutEventsSuccessResponse(data, eventMap) {
    ...
    Object.entries(Results).forEach(([_, endpointValues]) => {
      ...
      Object.entries(responses).forEach(([eventId, eventValues]) => {
        ...
        logger10.warn("Pinpoint event failed to send.", {
          eventId,
          name,
          message: Message
        });
  ...
  _retry(retryableEvents) {
    ...
    retryableEvents.forEach((bufferedEvent) => {
      ...
      if (bufferedEvent.resendLimit-- > 0) {
        logger10.debug("Resending event.", {
          eventId,
          name,
      ...
      logger10.debug("No retry attempts remaining for event.", {
        eventId,
        name
      });
    });
...
var getEventBuffer = ({ appId, region, credentials, bufferSize, flushInterval, flushSize, resendLimit, identityId, userAgentValue }) => {
  ...
  if ((_a = eventBufferMap[region]) == null ? void 0 : _a[appId]) {
    ...
    if (buffer3.identityHasChanged(identityId)) {
      buffer3.flush();
    } else {
      return buffer3;
    }
...
*/
//# sourceMappingURL=chunk-6F3VFPZ6.js.map
